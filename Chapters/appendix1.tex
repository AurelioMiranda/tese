...
\begin{description}
	...
	\begin{description}
		...
		\item[Include logic summaries:] \texttt{true}
		\item[Show detected errors:] \texttt{true}
		\item[Open automatically:] \texttt{true}
	\end{description}
	
	\item[Business Logic]
	\begin{description}
		\item[Decoupling:] \texttt{true}
		\item[Executor:] \texttt{SingleThread}
	\end{description}
	
	\item[Files]
	\begin{description}
		\item[Overwrite existing:] \texttt{true}
		\item[Output directory:] \texttt{./generated}
		\item[Header prefix:] \texttt{Telecom}
	\end{description}
	
	\item[Extension]
	\begin{description}
		\item[preGenScript:] \texttt{./script1}
		\item[postGenScript:] \texttt{./script2}
	\end{description}
	
	\item[Comments]
	\begin{description}
		\item[Enabled:] \texttt{true}
		\item[Copyright notice:] \texttt{(C) 2026 Telecom Project}
		\item[Author:] \texttt{John Doe}
	\end{description}
	
	\item[Traceability]
	\begin{description}
		\item[Enabled:] \texttt{true}
		\item[Produce diagram:] \texttt{true}
		\item[Create JSON:] \texttt{true}
	\end{description}
	
	\item[Build Integration]
	\begin{description}
		\item[Generate source files:] \texttt{true}
		\item[Generate Makefile:] \texttt{true}
		\item[Generate package file:] \texttt{true}
		\item[Generate launch file:] \texttt{true}
	\end{description}
	
	\item[Legacy Libraries]
	\begin{description}
		\item[Include:] \texttt{false}
		\item[Copy to project:] \texttt{false}
	\end{description}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.45\textwidth]{configLanguageUMLRotated.png}
...
Table~\ref{tab:ttpt-1} and table~\ref{tab:ttpt-2} describe the time taken by each participant on each task.

\begin{table}[htbp]
	\centering
	\caption{Usability Task Performance Summary (Tasks 1–10, Task Times in Seconds)}
	\label{tab:ttpt-1}
	\begin{xltabular}{\textwidth}{X X X X X X}
		\toprule
		\textbf{Task} & Participant 1 & Participant 2 & Participant 3 & Participant 4 & Participant 5 \\
		\midrule
		Select target language 				& 71.42 & 17.00 & 32.58 & 41.97 & 15.68 \\
		Select Linux as OS 					& 5.71 & 8.80 & 7.80 & 3.09 & 7.31 \\
		Set output directory 				& 12.57 & 13.80 & 18.61 & 11.36 & 48.77 \\
		Trigger code generation (1)			& 17.32 & 44.76 & 45.43 & 77.32 & 54.05 \\
		Configure naming style 				& 59.87 & 32.44 & 15.65 & 22.52 & 16.75 \\
		Apply identifier prefix/suffix 		& 22.13 & 22.00 & 65.87 & 16.13 & 38.68 \\
		Enable comments 					& 93.98 & 100.53 & 32.71 & 79.67 & 45.77 \\
		Enable report generation  			& 27.80 & 8.09 & 5.27 & 40.61 & 18.95 \\
		Enable logic summaries in report	& 9.41 & 3.95 & 7.10 & 9.05 & 3.91 \\
		Enable traceability 				& 6.40 & 41.83 & 6.11 & 19.36 & 33.19 \\
		\bottomrule
	\end{xltabular}
\end{table}
...
\begin{table}[htbp]
	\centering
	\caption{Usability Task Performance Summary (Tasks 11–20 and Totals)}
	\label{tab:ttpt-2}
	\begin{xltabular}{\textwidth}{X X X X X X}
		\toprule
		\textbf{Task} & Participant 1 & Participant 2 & Participant 3 & Participant 4 & Participant 5 \\
		\midrule
		Enable traceability JSON 			& 4.71 & 37.31 & 6.45 & 11.75 & 10.05 \\
		Toggle business logic decoupling 	& 18.77 & 5.88 & 10.96 & 12.51 & 25.36 \\
		Change executor type 				& 9.89 & 3.30 & 4.92 & 17.31 & 4.30 \\
		Configure legacy libraries 			& 8.99 & 27.19 & 14.53 & 39.21 & 17.38 \\
		Add pre/post scripts 				& 45.43 & 8.29 & 40.85 & 35.72 & 40.85 \\
		Set file prefix 					& 53.66 & 31.41 & 17.39 & 12.30 & 24.58 \\
		Enable file overwriting				& 8.10 & 4.76 & 4.25 & 13.42 & 3.02 \\
		Manage build file generation 		& 57.77 & 18.16 & 30.17 & 84.77 & 27.31 \\
		Trigger code generation (2)			& 19.55 & 32.67 & 25.89 & 23.54 & 23.88 \\
		Verify outputs 						& 39.06 & 57.64 & 27.47 & 41.96 & 48.64 \\
		\midrule
		\textbf{Totals} & 9m 52.54s & 8m 39.81s & 7m 00.01s & 10m 13.57s & 8m 28.43s \\
		\bottomrule
	\end{xltabular}
\end{table}
