...
\epigraph{
	Effective code generation requires not only automation but also adaptability to project-specific demands.
}
...
\begin{itemize}
	\item \textbf{Hardcoded Transformation Logic:} RAMSES' code generation policies—naming conventions, API selection, and coding style—are strongly embedded in the transformation rules themselves. This makes any alteration difficult and prone to errors.
...
This rigidity creates barriers to adoption in diverse industrial contexts where flexible code generation is critical.
