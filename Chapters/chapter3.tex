...
The implementation of the proposed \gls{DSL} requires a methodical and objective approach in terms of timing and tasks. For that reason, a Gantt chart was made with the intent of guiding the development and ensuring the end product meets expectations.
...
Client meetings are present to ensure that the features developed are useful and needed addition to the end user, the first and second meeting are mostly interviews to get quick feedback, while the third and last meeting will focus on the evaluation and testing of the mostly finished code generator configurator.

\subsection{Development Phase}
\label{sec:dev_plan_dev}

The development has an iterative nature, which means that, once a feature is developed, it is tested, documented and corrections are applied when needed. Active, full system testing, is also intermittently (once a month) done to ensure multiple features work in harmony with each other. A system integration test is also performed near the end of the development phase to ensure that end users can use the system effectively and can achieve goals unachievable before. This is a critical point in the development since it will be used in addition to the third client meeting to build a system evaluation report.

Parallel to the code generator configuration feature development, a \gls{DSL} will also be developed. This happens in parallel since, in order to use the featured properly we need to define a way to interact with the configurator. Near the end of a feature development we need to also implement its user interaction via the \gls{DSL}, hence its parallel development method which also includes being tested in the intermittent testing times. Both the feature and \gls{DSL} development go beyond the last evaluation and testing in order to ensure a safety gap for last minute features or corrections.

\subsection{Documentation Phase}
\label{sec:dev_plan_doc}

The final phase mostly consists in analysing the evaluation results and get insight on end user opinion, thought process and interaction when using the configuration language. Its a very important step since it will allow for quick analysis on implementation problems, bugs and other suggestions that might have not been included before. After that comes the documentation part for both users and future developers, since the configuration language will be very extensible. Documentation will also be done during development, however this interval serves to make sure everything is documented and ready to deliver.
...
The overall structure of the Gantt chart is clear and direct, although it provides very accurate time frames, in practice it will serve mostly as a base and the development might end up with additional or less phases/parts depending on the necessities of the parts involved. 
