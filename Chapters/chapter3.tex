...
\gls{RAMSES} has served as a robust model-to-code generator for \gls{AADL} based systems, yet it suffered from a fundamental architectural constraint: it assumes a uniform target environment. This assumption does not hold in real-world industrial projects, where system heterogeneity, legacy integration, and domain-specific standards define a constantly shifting context.

The core issue lied in \gls{RAMSES}' transformation pipeline: it entangled policy decisions (naming, structure, integration style) with generation logic. These decisions, hard coded in ATL transformations, reflect the assumptions of \gls{RAMSES}' authors more than the needs of end users~\cite{AtlasTransformationLanguage}. Altering them involves modifying the transformation source itself, often a hard and error-prone task~\cite{Mittal_Blouin_Bhobe_Bandyopadhyay_2022}.
...
Designing this kind of language is challenging. It needs to balance expressiveness, ease of use, and seamless integration. Based on industrial feedback and analysis of \gls{RAMSES} issues, the following characteristics are proposed:
...
\begin{itemize}
	...
	\item \textbf{Human-Readable and Tool-Accessible}: The configuration format (\gls{DSL}) should work well with version control, support difference review, and remain readable to engineers. It must also be machine-readable for validation and generation.
...
These characteristics make the configuration language robust, maintainable, and suitable for industrial use, allowing for flexible customization of code generation without compromising model integrity or introducing unnecessary complexity.
...
In order to test the functionality and receive a third party opinion, a simple user testing was done with two users, specifically \gls{RAMSES} developers that work directly with \gls{AADL}. The procedure was simple and direct, users were instructed to the premise of the prototype, then they would perform a task: change the \gls{identifier} name to X) and then they would be questioned in the usefulness of the feature, the applications it could have and overall thoughts.
...
Even though its application is specific, the code developed for the prototype is fairly generic. This means that most of the logic and code used for this prototype can be ported directly to \gls{RAMSES} without much issue, effectively implementing the feature, albeit \gls{RAMSES} specific testing was still performed.

The naming convention change logic is pretty straight forward considering \glspl{identifier} since those come from model properties. From there the properties that represent \glspl{identifier} are formatted as they are being added to the code, effectively making sure the main model stays untouched while the properties extracted are modified into the desired outcome. This makes sure only the specific selected properties are modified and the rest of the code generates as normal.
...
This requirement is fairly straightforward to prove since it implies a direct usage of the \gls{DSL} to produce results. This can be proven alongside the other requirements as they depend on it. However, a system wide testing quickly proves the existence of the configuration language and its impacts, specifically with the testing of all features developed and their results.
...
This setup will help assess whether the \gls{DSL} meets its usability and accessibility goals across a broad user spectrum.

\subsection{Configuration Variants}

To assess the flexibility and robustness of the configuration language, distinct configuration files will be tested automatically. These scenarios will vary in complexity and structure to evaluate whether the generator can handle diverse inputs while maintaining correctness and coherence in the generated code, effectively testing the limits of the configuration language.
...
\begin{itemize}
	\item Unit Testing which was performed by the developer. These tests focused on individual transformation rules or components of the generator to make sure developed features work as intended.
	...
	\item Integration Testing was also be performed by the developer and checked whether different features of the generator work together correctly. It focused on feature interactions and overall system behavior, especially when configuration options are combined.
