...
In order to better understand what features were implemented, when and why, the implementation was planned and executed by layers: each layer depends on things done on previous layers, hence some features were developed before others. 
...
The extension option represents the generation hooks (see Section~\ref{sec:impl_gen_hooks}) which were implemented with an already existing workflow component. This settings configure pre and post generation hooks that allow execution of external scripts. 

These hooks enable automation without modifying the code generator. 

\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{eclipse_config_extension.png}
	\caption{Page of generation hooks for code generation}
	\label{fig:eclipse_config_extension}
\end{figure}
