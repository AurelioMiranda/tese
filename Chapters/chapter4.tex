...
\epigraph{ 
	\textit{This chapter describes the technical implementation of the configuration language for RAMSES. It details how the insights from industrial collaboration informed the architectural decisions and how each feature was implemented.}
}
...
A structured requirements elicitation session was conducted with an industry partner to validate the proposed feature set and refine the architectural direction of the system. The objectives of the session were:

\begin{enumerate}
	\item \textbf{Feature Validation:} Confirm the relevance, priority, and feasibility of previously identified features (Identifiers, Traceability, Report generation).
	\item \textbf{Architectural Refinement:} Identify architectural improvements to enhance maintainability, scalability, and development efficiency.
	\item \textbf{Product Alignment:} Ensure consistency between the current development effort and the expected structure and functionality of the final \gls{RAMSES} product, including its user interface and dedicated code generation section.
\end{enumerate}

The session resulted in the validation of core functional features and the introduction of an architectural requirement regarding the separation of \gls{ROS}-specific and generic C++ code. Most of the previously thought of features (\Glspl{identifier}, Traceability, Report, etc) were endorsed, however, a new feature was suggested by an expert \gls{ROS} developer: the separation of \gls{ROS} specific code and C++ specific code, meaning that code that is specific to \gls{ROS} was to be isolated as it is very similar along the various generated files and it provides many benefits such as increased portability and clear separation of concerns. 

\begin{tcolorbox}[colback=green!5, colframe=green!40!black] 
	This meeting provided extremely valuable insight as the industry partner has been using \gls{ROS} for over seven years, effectively distilling their experience into actionable knowledge. 
\end{tcolorbox}
...
In order to better understand what features were implemented, when and why, the implementation was planned and executed by layers: each layer depends on things done on previous layers, hence some features were developed before others. Essentially, a layered architecture.
...
\begin{xltabular}{\textwidth}{X X X}
	...
	Target Operating\par System & Business Logic & Increases flexibility \\
	\bottomrule
\end{xltabular}


Table~\ref{tab:impl_plan} presents the planned order of feature implementation based on their dependencies. Some features required elements developed in previous steps for instance the Report which uses Traceability features. This dependency structure prevented rework by implementing features that provide necessary infrastructure before higher-level functionalities. This effectively mitigates the issues that arise when software features depend on each other. If the order was reversed the focus would need to constantly shift between features. The current ordering allows each feature to be developed with its dependencies already done and tested.


\subsection{Core and Metadata Layer}
\label{sec:code_and_meta}

The Core and Metadata Layer (Table~\ref{tab:core_and_meta}) includes foundational features that establish the basic code structure and metadata necessary for subsequent processing. Features such as \glspl{identifier} and comments form the backbone of the generator, while traceability and reporting provide essential tools for debugging and verification. Their relatively low to medium complexity allowed them to be developed early, enabling smooth integration of more advanced features later.
...
This first layer served as a basis for other features, forming the foundation of the systems layered architecture.
...
\begin{tcolorbox}[colback=green!5, colframe=green!40!black] Implementation describes how \gls{RAMSES} parses, interprets, and transforms the \gls{DSL} to affect produced code through its code generation mechanisms. \end{tcolorbox}


\subsection{Naming Conventions}
\label{sec:impl_name_conv}

The implementation of naming conventions was straightforward since the demo developed in Section~\ref{sec:proof_of_concept} was a substantial part of the complete mechanism. Using Acceleo we are able to modify the properties coming from the model as they are added into the C++ file. In this case, the name is formatted according to the selected convention. The main addition to the prototype work was the introduction of dynamic naming conventions that depend on the selected language. For example: in C++, classes are declared in PascalCase, whereas variables are declared in snake\_case.

\begin{lstlisting}[language=Java, caption={Generated Code of the ImageDisplay Component}, label={lst:imgdisplaycomponent}]
...
As shown in the generated code in Figure~\ref{lst:imgdisplaycomponent}, \texttt{ImageDisplay} and \texttt{image\_subscriber\_sub\_} are both \glspl{identifier} but each one has a different format. In contrast, the previous implementation without configuration would be \texttt{Image\_Display} and \texttt{Image\_Subscriber\_Sub\_}. These names come directly from the \gls{AADL} model as \textbf{\texttt{Image\_Display} and \texttt{Image\_Subscriber}}.
...
This implementation is central, as the remaining features depend heavily on it. For this reason, it was developed first. Additionally, it improves readability and enforces language-specific best practices, which are critical in mixed-language projects (in the case of \gls{RAMSES}: C++ and \gls{ROS}).

\begin{tcolorbox}[colback=green!5, colframe=green!40!black] 
	Note: There is a possibility that two names collide if they have the same naming structure (say ImageDisplay and imageDisplay). However, this would stem from a bad system design practice which \gls{AADL} discourages as identifiers are case-insensitive.
\end{tcolorbox}
...
The implementation of dynamic code comments allow the user to get much more context on the code while preserving the ability to view raw, non-commented code. The way it works is pretty straightforward. The user can toggle the comments on or off. Comments are dynamic according to their component and only provide essential info.
...
\begin{lstlisting}[language=XML, caption={Generated Traceability JSON of the Robot\_Vision\_I\_Instance system}, label={lst:RobotVisionIInstance}]
{
...
As shown in the code from Figure~\ref{lst:RobotVisionIInstance}, the \gls{JSON} relates the model to the code in various forms, mainly code traces, qualified names, file and package locations. On top of that, it provides additional info that is useful for both developers and end users. Developers can easily use this \gls{JSON} as input for various tools (generating an alternate report from the one provided for example). Users can use it to trace parts of the code dedicated to a specific component or subcomponent using a combination of the file locations, code traces and line number.

The way the line number is detected is as follows:

\begin{itemize} 
	\item When traceability is enabled, the generated code includes trace markers. These markers are language-agnostic and are not interpreted as part of the target language syntax or processed by external tools.
	\item The same trace information is also exported to a \gls{JSON} file, which serves as a structured representation of trace metadata.
	\item During post-processing, the \gls{JSON} data is used to identify traces associated with subcomponents of a larger component (represented as a C++ file). The corresponding C++ files are then scanned for trace markers, and when a match is found, the relevant information is recorded alongside the trace, linking it to the corresponding line in the generated code.
\end{itemize}

Figure~\ref{fig:trace_flochart} describes visually the traceability process.

\begin{figure}[htbp]
	\centering
	\includegraphics[height=0.22\textwidth]{trace_flowchart.png}
	\caption{Flowchart describing the traceability process (made by the author)}
	\label{fig:trace_flochart}
\end{figure}

Using \gls{JSON} simplified a lot the process of assigning the correct line to the correct trace since \gls{JSON} can be processed with Java code in a simple and effective way. 

Each complete component represented in the \gls{JSON} adds roughly 1Kb of size to the file. When the component simply points to an external library and not to a generated code file, that value reduces drastically, meaning that a complete system with 6 components can be represented by a \gls{JSON} trace file with 5-6Kb when no external libraries are present or 1Kb if every component has its code in an external library. The overall \gls{JSON} file size is very modest and should not cause storage issues.

In addition to this, traceability links were created using in the report (see Section~\ref{sec:impl_report}) to ensure that the \gls{JSON} file can be directly related to the C++ files using the \textit{@trace} indicative.
...
\begin{itemize} 
	\item \textbf{PDF with Java:} The major advantage that producing \gls{HTML} reports with Acceleo fixes is that producing PDFs with Java code, although more practical\footnote{Less technologies involved. Just Java as opposed to Java, Acceleo and \gls{HTML}} on the surface, it has many limitations when it comes to designing and structuring PDFs. In addition, the instructions were very unintuitive and can quickly grow in complexity.
	...
	\item \textbf{ Markdown vs HTML:} While Markdown offers a simpler syntax, it is intentionally limited in terms of layout control and styling capabilities. Generating structured reports with fine-grained formatting, custom styling, and dynamic elements would require additional tooling (\textit{e.g.}, conversion pipelines) or embedded \gls{HTML}, which reduces its simplicity. In contrast, \gls{HTML} combined with \gls{CSS} provides full control over document structure and presentation, making it more suitable for generating customizable and extensible reports in this context.
\end{itemize}
...
Since the first iteration of the report was done in \gls{HTML}, the transition to PDF was not needed since Eclipse naturally opens and displays \gls{HTML} files in a selected browser. If there was, however, a need for an \gls{HTML} to PDF transformation it could easily be done via workflow\footnote{More on workflows in Section~\ref{sec:workflows}.} after the PDF is generated.

The report has many features. Its high customization enables the creation of different parts when needed, hence omitting redundant or unwanted sections according to user needs. One of the core elements is the \gls{ROS} specific option, which highlights \gls{ROS} specific components that were generated or ported from the model. This allows \gls{ROS} developers to have a much clearer idea of how parts of the input model were represented in the \gls{ROS} part of the code, for instance, what thread corresponds to a Publisher or Subscriber, since \gls{ROS} uses a Publisher-Subscriber architecture~\cite{5645557}.

The report takes a longer time of generation when comparing to other parts of the generation (more on execution times in Section~\ref{sec:exec_times}). This is because it also includes error reporting, which analyses every code file for errors.

This functionality is loosely tied with the report in order to better display the results of the Cppcheck tool, which will be explained in Section~\ref{sec:impl_code_qual_checker}. However, since the output of Cppcheck is difficult for end users to interpret, since it comes in XML format, the best way to display it is using the report.

The contents of this report give insight on the generated code. This content goes into detail about the technologies, build system and models used to generate the code, giving the timestamp of generation and the output folder as well. Along with this, a navigation section also gets generated which facilitates the finding of a specific component or section. Before the components, a short summary of the generation is displayed with info such as number of generated files, threads, code language and configurations applied. Every single process (originated from the \gls{AADL} model) is documented in the components section, detailing the file they are located on and the threads managed by them. Optionally a \gls{ROS} process summary might be generated as well for each component. 
...
An addition to the code generator was the introduction of a code quality checker powered by \textit{Cppcheck}, which supports the analysis of every generated code file an points out errors immediately after generation. This allows the end user to quickly identify syntax errors and common code problems. This certainty is based on the capability that \textit{Cppcheck} has to report code errors. It does not guarantee that the code is semantically functional, just syntactically correct.

Moreover, the code can be analyzed at any point, meaning that \textit{Cppcheck} can verify the correctness of user generated code if needed, providing value even after being used for its main purpose.

\textit{Cppcheck} is a static analysis tool for C and C++ code. It is designed to detect a wide range of programming errors, potential bugs, and code style issues without executing the program. \textit{Cppcheck} analyzes the source code to identify problems such as memory leaks, uninitialized variables, unused functions, and violations of coding best practices. It is a lightweight, platform-independent, and highly configurable tool, which makes it suitable for integration into our code generation workflow.
...
To mitigate this, a new approach to code generation was implemented. The main goal was to port the \gls{ROS} code inside the C++ files onto another folder, adjacent to the remaining C++ files. This can be better understood visually. We part from Figure~\ref{lst:folder}.

\begin{lstlisting}[language=C++, caption={Basic folder structure example}, label={lst:folder}]
src/
...
And we split the files into their \gls{ROS} and C++ counterparts, as shown in Figure~\ref{lst:foldersplit}

\begin{lstlisting}[language=Java, caption={Split folder structure example}, label={lst:foldersplit}]
src/
	 impl/
...
With the files now separated, it is possible to reuse the C++ code present in the \texttt{impl} folder without the \gls{ROS} dependencies. This also opens more the window to the inclusion of company specific libraries that can, if needed, be split into \gls{ROS} and C++ only counterparts.
...
This file header can be any phrase or characters but due to operating system restrictions, some character have to be filtered out. An example of this filtration process can be shown in Figure~\ref{lst:file}.

\begin{lstlisting}[language=Java, caption={File name filtering example}, label={lst:file}]
...
A good example of the result of this file header implementation is, based of the files in Section~\ref{sec:impl_ros_separation}, when adding the header \texttt{Nova"}, the result is as shown in Figure~\ref{lst:file_done}

\begin{lstlisting}[language=Java, caption={File structure with header example}, label={lst:file_done}]
	src/
		 impl/
...
Note that the quotation mark is transformed into an underline as mentioned previously.
...
When overriding is enabled, the generator does not change. It still generates every file every time. When it is disabled, however, it only generates files that are not yet present. For instance, if we have a complete project, no new files will be generated or substituted. In contrast, if one file is missing, the generator will generate only the missing file. Preliminary tests indicate that disabling overriding can reduce execution time. With up to 500ms less than the override enabled counterpart, proving that it is worth it to give the user the choice between generating the whole project or just specific parts of it. These measurements were obtained by running the generator multiple times under identical conditions. However, the magnitude of this improvement depends on the project size and number of files regenerated.


\subsubsection{Generation Hooks}
\label{sec:impl_gen_hooks}

Generation hooks are also present as suboptions of file management, this is due to the need to run certain scripts before or after project execution. These can include cleanup scripts to prepare the project folder for a new version or post generation scripts to automatically test the generated project, these scripts can bring value and a higher degree of automation to any project.

For those reasons, generation hooks were included in the configuration language. This feature is implemented by allowing users to specify pre- and post-generation actions within the configuration language, which are then invoked at defined points in the generation process.

The integration of generation hooks leverages the existing workflow execution mechanism, which already supports extensibility at different stages of the generation pipeline. More technical details can be found in Section~\ref{sec:workflows_script_executor}.
...
To put it simply, in order to produce the functionality that the input model entices, certain legacy libraries need to be included, not just in the project, but called in the code as well. This was done as follows:
...
For this to work it is crucial to accurately detect which components already have libraries associated to them. This comes before the component is even instanciated, meaning: when we write the \gls{AADL} system we want, the system components already have libraries associated to them. This is done by explicitly including the libraries in the components properties.

The baseline code generator\footnote{The non configurable version of the code generator, done by António.} already somewhat entailed to this functionality by not generating a skeleton when the library property was present. However, it lacked actually referencing that library properly and including it in the project.

The solution to this was straightforward: to include the libraries (when the configuration required it) and their reference properly in the generated code.
...
This inclusion of the legacy code libraries was of extreme importance since it was one of the core goals of the thesis (see Section~\ref{sec:obj_and_contrib}) and it is a crucial step to a complete \gls{ROS} project generation.
...
The main problem is that, if we want to create and customize a new configuration we have to start from scratch and use a new file. To mitigate this challenge, a new way of creating configurations was introduced: the list of configurations.
...
The solution to this is to add to the already existing enumerator but intrinsically, this requires modifying the current metamodel.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.3\textwidth]{beforeLanguageChange.png}
	\caption{Class diagram representing the base idea of choosing a coding language}
	\label{fig:beforeLanguageChange}
\end{figure}

Figure~\ref{fig:beforeLanguageChange} represents the list of languages before the change. Since the languages are represented by an enumeration, the addition of a new language involves coding and regenerating the configuration model.

\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.3\textwidth]{afterLanguageChange.png}
	\caption{New, improved class diagram representing the selection a coding language}
	\label{fig:afterLanguageChange}
\end{figure}

As we can see in Figure~\ref{fig:afterLanguageChange}, since a coding language is now represented by a class, it is now possible to add new languages without modifying the metamodel, which makes the process more seemless and able to be automated so the end user can add their desired coding languages with a simple menu instead of having to write code.

The \gls{DSL} architecture suffered a tiny shift which ended up changing the way single file configurations work. Since now a GeneratorConfig requires a reference to a Language, this Language needs to be imported via \textit{Load Resource...} to be associated with the configuration. This of course, only applies to single configuration instances since the multiple configuration architecture has a list of coding languages.
...
Although simple, this addition is very useful and, once again, adds to customization and control over the generated files. The files previously mentioned are set to be generated by default.
...
\begin{itemize} 
	\item Multiple Java classes are required, each responsible for generating part of the final artifacts.
	\item The complete generation of a \gls{ROS} project requires invoking several Java methods distributed across these classes.
	\item Certain generation steps depend on the outputs of previous steps, enforcing a strict execution order (\textit{e.g.}, the \gls{JSON} traceability file must be produced before it is populated with the line traces).
	\item These methods cannot be run in parallel, as that would require a complete redesign of the current architecture making it less flexible.
\end{itemize}


The solution for these issues is workflows. Workflows allow the automation of certain tasks (reading, writing, or transforming models) by plugging together several workflow components, which are executed consecutively. A workflow component performs a certain task and can be configured with parameters.
...
This component is optimized to every single Acceleo Java class present in \gls{RAMSES}, meaning that, in the workflow, every Java class can be represented by an AcceleoGeneratorComponent which facilitates the sequential execution and removes the need to have components specific to a certain class.

In addition to solving the previously mentioned issues, this new component was also engineered to run functions before and after generation if those functions existed in the input class. As an example, an input template AT (with an associated class AC) can have as methods \textit{shouldRun()}, \textit{preGeneration()} and \textit{postGeneration()} on top of the generic method \textit{doGenerate()}. These methods have key functions that help enrich the component beyond simply executing a class, namely:
...
In addition to all of these features, this custom component also simplifies changing the inputs of the generation. Since there is a property for each of the inputs in the workflow (\gls{AADL} model, configuration model, working directory and output folder), the change in the AcceleoGeneratorComponents is as follows: a change to the property will affect all the workflow components that use that property.


\subsection{Dynamic Acceleo Workflow Executor}
\label{sec:workflows_dynamic_executor}

The custom component described in the previous Section~\ref{sec:workflows_acceleo} fulfills main purpose. However, a key issue arose: instead of feeding the template to the model, we feed the class that that template represents. This is an issue since if we want to execute a specific Acceleo template we need its class.

The solution to this problem was as follows: instead of calling the classes independently, the component would call a generic class that would handle any Acceleo template. This did not come without drawbacks. In particular, that handling every template the same way reduced their overall specialty. This was an issue that the previous component solved by integrating additional functionality in each individual class as needed.

Essentially the two architecture alternatives where either full specialization but low integration capabilities or high integration and low specialization. However, this framing overlooks a third alternative: achieving both through a different architectural approach.
...
One of the features mentioned both in the feature model~\ref{fig:feature_model} and in the client meeting~\ref{sec:industry_meeting} was the ability to execute pre or post generation hooks as discussed in Section~\ref{sec:impl_gen_hooks}. 
...
\begin{itemize} 
	\item \textbf{Arguments: } $\backslash$c, pregen.bat\footnote{This represents the executable that we want to run.}
...
The Launch, CMake and Package files take much less time than the other ones due to their simplicity and to the fact that they generate a single file, as opposed to the Code generation which, in this case, generates 7 C++ code files, each corresponding to a model component. The report takes even longer than that because, on top of documenting the generated files and build an HTML report, it also checks those files for code quality issues, which ends up taking even more time. The JSON Trace file is pretty straightforward. However, since it scavenges the code files for the exact placement of the traces, it does end up with an execution time bigger than the other single file generations.

Since the base project did not have a workflow that successfully implemented the baseline Java code, it is not possible to directly compare the execution times of the old version of the code generation with the new one. However, we can estimate the time it took to execute a project generation in the baseline generator with the information from Table~\ref{app:workflow_exec_time}, which, after measuring the time it takes to generate the Launch, CMake, Package and C++ files, comes around to 231,6ms, essentially half of the current average execution time of 479,9ms. This is expected since both the Trace and the Report take a significant amount of time. This interval can be even bigger if we consider that the original code generator did not even have a configuration file, and therefore supported a more limited set of features. As a result, the increase in execution time reflects the additional functionality of the current generator rather than a loss of efficiency.
...
As mentioned previously, Figure~\ref{fig:avg_exec_times_1} confirms that the average execution time seems to have doubled with the introduction of more processing heavy modules (Trace and Report). This is not a big issue as the total execution time is still below 500ms which is still very acceptable for the current conditions and given the increased amount of functionality and dynamicity added to the code generator.

All in all, workflows proved to be an essential part of the code generation pipeline that tie together the functionality of every individual configuration option in a single, harmonized execution. In practice however, the editing of the configuration and the execution of the workflow will not be done in the raw files, but in \gls{Osate} using buttons and preferences for execution and configuration respectively.
...
It is independent from but used by \gls{RAMSES}. \end{tcolorbox}


\subsection{Custom Preference Store Component}
...
In the case of the code configuration \gls{DSL}, this preference window fits perfectly with the language. It provides a way for users to modify the options freely while also restricting and minimizing possible errors since the \gls{UI} presented has guardrails that prevent the user from introducing errors into the \gls{DSL}.

This approach for the \gls{UI} however came at a cost: the storage of values was already implemented by eclipse, meaning that, by default, it was not possible to save the \gls{DSL} metamodel whole, just a key:value map.
...
This essentially means that the manipulation of the very base of the \gls{DSL} itself can now be saved in Eclipse's preferences, allowing it to be used instantly.
...
This model then can be used as the input for Workflows (Section~\ref{sec:workflows}) to generate code based on its configuration, effectively giving the user control over the base metamodel while also restricting his possibilities to commit errors.

This storage approach does not affect the \gls{UI} but it does simplify and improve the overall process of managing the \gls{DSL} metamodel.
...
These options are strictly necessary for code generation to take place, hence being the main options. All options presented in this Section~\ref{sec:dsl} are available to be configured under \textit{\gls{Osate} > Window > Preferences > CodeGen Config} in the \gls{Osate} platform.
...
Figure~\ref{fig:eclipse_config_main} highlights how the main configurations appear in the preferences window. From here it is possible to select the output folder and the target language that will be fed to the generator for code generation.
...
The essence of the naming convention option is simple: to automatically change the casing on \glspl{identifier} to ensure consistency across all generated artifacts. It allows the user to enforce casing rules on classes, methods, variables, and constants throughout source files.
...
Most styles simply change every \gls{identifier} to the same style. Examples of this are \texttt{snake\_case}, \texttt{PascalCase}, \texttt{camelCase} and many others. The \texttt{DEFAULT} option leaves identifiers exactly as they have been written in the model, allowing for an effective nullification of the formatting process.

The \texttt{STANDARD} naming convention differs from the others as it allows for a dynamic \gls{identifier} naming, for instance: it allows \texttt{Classes} to be \texttt{Pascal\_Case} while simultaneously having \texttt{Variables} in \texttt{snake\_case}. This is very useful for specific restrictions where languages need different casing based on \gls{identifier} (Classes, methods, variables, constants, etc). This specific casing is very configurable and can be adapted to different programming languages, further improving the adaptability of the \gls{DSL}.


\subsubsection{Identifier Affix}
\label{sec:dsl_name_affix}

Users may additionally define a prefix or suffix to be applied to all \glspl{identifier}
...
Enforcing identifier casing is especially important in parts of the toolchain where naming conventions may be interpreted semantically, such as ROS message fields and class names. Having the style configuration in the DSL ensures that all generated artifacts are syntactically consistent with the selected target environment.

%TODO: review the phrase above
...
Figure~\ref{fig:eclipse_config_business} highlights the business logic menu options.
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{eclipse_config_files.png}
	\caption{Page of the file management configuration}
	\label{fig:eclipse_config_files}
\end{figure}

Figure~\ref{fig:eclipse_config_files} shows the options that control the file management.

The extension option represents the generation hooks (see Section~\ref{sec:impl_gen_hooks}) which were implemented with an already existing workflow component. This settings configure pre and post generation hooks that allow execution of external scripts, effectively enabling automation without modifying the code generator. 
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{eclipse_config_comments.png}
	\caption{Page of the code comments configuration}
	\label{fig:eclipse_config_comments}
\end{figure}

Figure~\ref{fig:eclipse_config_comments} illustrates the different code comment generation options.
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{eclipse_config_trace.png}
	\caption{Page of the traceability configuration}
	\label{fig:eclipse_config_trace}
\end{figure}

Figure~\ref{fig:eclipse_config_trace} shows the different options for traceability, as we saw previously with the case of the report generation (Section~\ref{sec:dsl_report}), if traceability generation is disabled, dependent settings become inactive. This behavior can be seen in Figure~\ref{fig:eclipse_config_trace_disabled}

\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{eclipse_config_trace_disabled.png}
	\caption{Page of the traceability configuration with traceability disabled}
	\label{fig:eclipse_config_trace_disabled}
\end{figure}

\subsection{Build Integration}
\label{sec:dsl_build}

Build integration controls whether main project artifacts such as \textit{source}, \textit{make}, \textit{package} and \textit{launch} files are generated. 
...
Figure~\ref{fig:eclipse_config_build} illustrates how the options can be selected. Each one of the options is independent from each other, meaning that they can be ticked on/off in any combination.
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{eclipse_config_legacy.png}
	\caption{Page of the legacy configuration}
	\label{fig:eclipse_config_legacy}
\end{figure}

Like other previous pages, the legacy page requires the inclusion of legacy libraries for the copy option to be enabled. Figure~\ref{fig:eclipse_config_legacy} illustrates how the page looks with all the options enabled and Figure~\ref{fig:eclipse_config_legacy2} shows how the the copy option depends on enabling library inclusion, ensuring configuration consistency.
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=\textwidth]{eclipse_config_legacy2.png}
	\caption{Page of the legacy configuration with library inclusion disabled}
	\label{fig:eclipse_config_legacy2}
\end{figure}
...
This option is one of the core options and can be found in Figure~\ref{fig:eclipse_config_main}.
