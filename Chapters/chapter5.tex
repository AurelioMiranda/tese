...
\begin{itemize} 
	...
	\item RAMSES2 version Aug 26, 2025
...
\begin{xltabular}{\textwidth}{X X X X X}
	...
	TC-INT-06 & Cross-Feature Stability & Test large models with custom \glspl{identifier}, comments, and style violations. & All features work together without breaking. \\
\end{xltabular}
...
\begin{itemize}
	...
	\item \textbf{TC-INT-06 - Cross-Feature Stability:} Used a large, complex model with:
	\begin{itemize}
...
The testing environment remained mostly the same, with the exception of the execution platform: testing was moved from a physical Ubuntu installation on Windows to a dedicated Linux virtual machine. The \gls{RAMSES} version was also updated to Oct 27, 2025.

The initial tests were conducted with a "parameterized" way in mind, meaning that the architecture prioritized code efficiency albeit with an addition of complexity. This in of itself is not an issue at all, the tests were still very efficient, accurate and highly maintainable, since to add a new test, at least one function needs to be added. However, the current \gls{RAMSES} architecture benefited more from running tests in an isolated, standalone form, meaning that it would be better if it was possible to run specific test of the generated code without the need to run the whole test strip. For this goal, the parameterized way of testing was inefficient, since it entails that every test is generic and there is no way to quickly modify specific parameters in a single test.

So, the test class was re-written to have one method per test, per model. This means that instead of ten generic tests run with parameterized values, there would be fifty specific tests, ten for each test model, given that five models were considered at that point in time.
...
An obvious flaw of this model is that, not only does it need complete manual validation in the start, but it's also very rigid when it comes to file comparison, comparing each file line-by-line\footnote{The comparison is not overly rigid and allows for exceptions such as the generation timestamp and order changes in \gls{XML} files.}.
...
The tests described in table~\ref{tab:reg_tests_1} passed via \gls{JUnit} tests, which not only were able to verify the present tests but also additional ones that are not present in this table. The full detailed list of tests can be found in the Appendix~\ref{app:unit_tests}.

TC-REG-02, TC-REG-03 and TC-REG-04 passed the unit test strip. This process can be automated and viewed quickly. TC-REG-01 added a layer of complexity because it pushed for manual verification which had to be done to ensure the functionality of the output code remained the same as the control group.
...
With the \gls{DSL} finalized and its features implemented it was then possible to assess how an end user will interact with the newly developed features. The object of study was the developed \gls{DSL} and configuration workflow it uses. The main purpose of this usability validation test was to evaluate, understand, and improve the code configuration \gls{DSL} and workflow.

This testing focused particularly on user efficiency (time to complete tasks) and effectiveness (task completion rate) of accomplish tasks, as well as how fast they learned the configuration itself, how often mistakes were made and how comfortable they feel using it. This information will not only be valuable to the current thesis but will also be taken into account when developing the rest of the \gls{RAMSES} tool with an integrated place for the configuration language.
...
The tests were conducted using the \gls{DSL} within the \gls{RAMSES} code generation workflow, operating in a Windows laptop with \gls{Osate} with the latest versions available at the moment of testing (see more on Section~\ref{sec:test_usabl_eval_crit}), using it with functional \gls{AADL} models.

In \gls{GQM} terms, this means analyse the developed DSL and its configuration workflow for the purpose of evaluating, understanding, and improving the workflow with a quality focus on usability attributes, including efficiency, effectiveness, learnability, error rate, and user satisfaction from the perspective of end users with moderate \gls{AADL} knowledge and highly experienced \gls{ROS} developers in the context of the \gls{RAMSES} code generation workflow using functional \gls{AADL} models.
...
Given the availability constraints inherent in specialized domains such as \gls{AADL} and safety-critical modeling, the participant group for this study constitutes a convenience sample. According to Falessi \textit{et al.}~\cite{Falessi_Juristo_Wohlin_Turhan_Münch_Jedlitschka_Oivo_2017}, convenience sampling is acceptable in empirical software engineering studies when the objective is exploratory evaluation, early-stage usability validation, and identification of practical issues affecting real users. However, the limited number of participants in this study does not allow for statistically significant conclusions or safe generalization to the general population. Therefore, while this sampling approach restricts external validity, it remains suitable for providing indicative insights into typical user behavior within the intended user demographic.
...
The number of participants was not random. Many studies have shown that there are conflicting opinions regarding the perfect amount of users needed to verify the usability of an application. Carol Barnum~\cite{Barnum_2003} strongly concludes that Nielsen's~\cite{Nielsen_2024} 5 user minimum is not adequate for many full fledged applications and system diversity, user diversity, test goals and risk tolerance can help decide a more appropriate amount. That same principle applies here but in a different manner because we are not simply testing a complete application but only a specific integrated tool that is part of the broader system. It is correct to say that more users will yield better overall results.

Out of all participants, only one of them interacted with a very early and incomplete version of the \gls{DSL} and, during the test, this participant did not entice any familiarization with the current \gls{DSL}. Overall, participants were not familiar with the \gls{DSL} prior to the evaluation.
...
The participants were not blinded to the end goal, which was presented as a "usability validation of a new code configuration workflow for \gls{RAMSES}." However, they were provided no task-specific guidance and were told the purpose of the test was to evaluate the tool, not the user. This minimized anxiety and the potential for a strong Hawthorne effect, which states that people behave differently when they know they are being watched~\cite{McCambridge2014Hawthorne}.


\subsection{Experimental Materials}
\label{sec:test_usabl_materials}

To ensure the test was valid and reproducible, the technical environment and all its artifacts were the same across all testing sessions. This controlled setup reduces variability introduced by environmental factors, thereby supporting fair comparison between runs. Detailed documentation of the experimental setup was provided to support reproducibility, in line with the FAIR principles for scientific research~\cite{fair}. The objective was to create a representative and consistent workflow environment for \gls{RAMSES} users.
...
\textbf{\gls{AADL} Editor}: \gls{Osate} version 2.16.0.vfinal, based on Eclipse 2024-03 (4.31). This serves as the primary development environment for \gls{AADL} model authoring and \gls{RAMSES} plugin execution.

\textbf{Code Generation Tool}: The \gls{RAMSES} Code Generator, specifically the commit version Dec 4 2025, was utilized as an \gls{Osate} plug-in. This component is responsible for processing the \gls{AADL} model alongside the \gls{DSL} configuration file to produce the target code.
...
Although it was not possible to test the entirety of the \gls{DSL} due to time, fatigue and stress constraints, the few options from every category means that it was possible to simulate a scenario close to real/industrial. In particular, configuration options such as the target language selection were prioritised over less critical options, such as the inclusion of a copyright notice in the code comments section, in order to maximise efficiency within the available time. The detailed task booklet can be found in the Appendix~\ref{app:task_book}.
...
The interaction with the \gls{DSL} is being treated as the independent variable. To operationalize the usability attributes, the task completion time, number of errors and help requests were monitored.
...
\begin{itemize}
	\item \textbf{Task completion time (Efficiency):} Operationalized as the duration in seconds from the moment the participant started reading the task prompt to the moment the configuration was successfully applied.
	\item \textbf{Number and type of errors committed (Reliability):} Operationalized by counting the number of validation errors, syntax mistakes, or logical misconfigurations triggered during the execution of a task.
	\item \textbf{Number of help requests or clarifications needed (Learnability/Support):} A count of explicit requests for clarification or guidance, used to identify areas where the \gls{DSL} or its documentation lacked clarity.
\end{itemize}
...
Qualitative observations were recorded through direct note-taking and screen recording during each session. Notes and recordings were reviewed post-session and annotated per task, allowing observations to be directly linked to specific configuration steps and usability events.
...
The \gls{SUS}~\cite{Brookesus} questionnaire was scored following the standard procedure defined by Brooke and the NASA-\gls{TLX}~\cite{HART1988139} questionnaire was administered using the unweighted scoring method. Participants rated each of the six workload dimensions (mental demand, physical demand, temporal demand, effort, performance, and frustration) on a 0–100 scale. The overall workload score was computed as the arithmetic mean of the six dimension scores. The unweighted approach was selected to reduce participant burden and improve consistency across sessions, which is appropriate for exploratory usability evaluations with a limited sample size.
...
Before the actual usability tests were conducted, a pilot test with a software developer was conducted as a means to identify possible errors in the workflow and to test the overall procedure. From the pilot test a clear point came across: monitoring task completion time, errors performed and help requests simultaneously was very highly complex. As such, all of the actual usability tests were also screen recorded in order to keep a clear and indisputable log of the test.

To ensure consistent handling between sessions the participants were tested individually in an isolated ambient with the software ready for use with the default settings. The procedure description and an example video was shown to every participant before the start of the test and the task booklet had the exact same tasks for each participant, albeit with randomized task orders. The mediator acted the same way for every single participant. No notable deviations were taken from the predefined procedure for any participant.
...
The information on Table~\ref{tab:usability_tasks} is averaged between all test users on completion time and summed on help requests. The task completion times per individual user can be found on Tables~\ref{tab:ttpt-1} and ~\ref{tab:ttpt-2}.

With all data collected, we can proceed with the analysis.

The analysis followed a descriptive and exploratory approach. Given the small sample size ($n=5$) and the primary goal of usability validation rather than hypothesis testing, no statistical tests were applied. Instead, quantitative metrics were analyzed using the aggregated descriptive statistics, including average task completion times, standard deviations, and observed minimum and maximum values, alongside total counts of help requests.

A task was considered to have acceptable efficiency if it could be completed by most participants within a time frame consistent with the observed average for that task, taking into account that all users were first time users of the configuration workflow. Tasks requiring textual input or multiple configuration steps were expected to take longer than simple selection or toggling actions and were therefore interpreted relative to task complexity rather than using a fixed time threshold.

With all data collected, we can proceed with the analysis.

\begin{table}[htbp]
	\centering
	\caption{Usability Task Performance Summary (with Dispersion Metrics)}
	\label{tab:usability_tasks}
	\begin{tabular}{lccccc}
		\toprule
		\textbf{Task} & \textbf{Avg (s)} & \textbf{Std Dev} & \textbf{Min} & \textbf{Max} & \textbf{Help Requests} \\
		\midrule
		Select target language 				& 35.73 & 21.86 & 15.68 & 71.42 & 2 \\
		Select Linux as OS	 				& 6.54  & 2.08  & 3.09  & 8.80  & 2 \\
		Set output directory 				& 21.02 & 14.79 & 11.36 & 48.77 & 1 \\
		Trigger code generation (1)			& 47.78 & 21.45 & 17.32 & 77.32 & - \\
		Configure naming style 				& 29.45 & 17.36 & 15.65 & 59.87 & - \\
		Apply identifier prefix/suffix 		& 32.96 & 19.42 & 16.13 & 65.87 & 2 \\
		Enable comments 					& 70.53 & 27.57 & 32.71 & 100.53 & 2 \\
		Enable report generation			& 20.14 & 14.05 & 5.27  & 40.61 & 1 \\
		Enable logic summaries in report	& 6.68  & 2.63  & 3.91  & 9.41  & - \\
		Enable traceability					& 21.38 & 15.58 & 6.11  & 41.83 & - \\
		Enable traceability JSON			& 14.05 & 12.66 & 4.71  & 37.31 & - \\
		Toggle business logic decoupling 	& 14.70 & 7.51  & 5.88  & 25.36 & - \\
		Change executor type 				& 7.94  & 5.47  & 3.30  & 17.31 & 2 \\
		Configure legacy libraries 			& 21.46 & 11.61 & 8.99  & 39.21 & 1 \\
		Add pre/post scripts 				& 34.23 & 14.97 & 8.29  & 45.43 & 4 \\
		Set file prefix 					& 27.87 & 16.27 & 12.30 & 53.66 & - \\
		Enable file overwriting 			& 6.71  & 3.98  & 3.02  & 13.42 & - \\
		Manage build file generation 		& 43.64 & 26.33 & 18.16 & 84.77 & 3 \\
		Trigger code generation (2)			& 25.11 & 4.82  & 19.55 & 32.67 & - \\
		Verify outputs 						& 42.95 & 10.78 & 27.47 & 57.64 & - \\
		\midrule
		\textbf{Totals} 					&  &  &  &  & 17 \\
		\bottomrule
	\end{tabular}
...
Table~\ref{tab:usability_tasks} illustrates the aggregated performance summary of the participants.

The completion times recorded are generally acceptable considering that all participants were using the configuration workflow for the first time and the test targets exploratory usability assessment. As expected, the first tasks tended to take longer as participants became accustomed to the interface, terminology, and behavior. Learnability was measured by observing the decrease in completion time, pauses, and help requests for repeated or similar tasks. Specifically, the repeated \textit{Trigger code generation} which went down from 47.78s to 25.11s, is a good example of a short term learning effect, where nearly all participants completed the second task much faster than the first, indicating a rapid gain in tool proficiency.

Textual input and multi-step configuration tasks tended to have higher completion times. For example, the \textit{Code comments} task required both finding the corresponding configuration option and completing a writing subtask (\textit{setting the author}), thus increasing the task time with an average total of 70.53s. Similar observations were made for tasks such as \textit{Apply identifier prefix/suffix} and \textit{Set file prefix}. As a consequence, these completion times are attributed to task complexity and input effort rather than usability issues.

Help requests were considered as indicators of usability issues rather than user errors. A clear hotspot of help requests was noticed for tasks associated with generation hooks/scripts and build file management, which combined accounted for 7 of the 17 total help requests. These requests tended to be of a navigational or conceptual nature, due to the fact that many of the participants had difficulty understanding the nomenclature used, particularly the concept of \textit{Build Integration}\footnote{Which encompasses Make, Package, Source, and Launch files.} and the \textit{Extension} hooks. However, as the help requests tended to be of a consistent nature across many of the participants, they were taken as an indication of the poor terminology used. As a direct result of the above observations, the option of \textit{Build Integration} was replaced by the option of \textit{Build}, and the \textit{Extension} hooks have been merged into the \textit{Files} configuration section.
...
Error categories were derived post-hoc from observed participant behavior. No predefined error taxonomy was decided prior to the testing sessions. Instead, observed errors were noted during and reviewed after each session. Afterwards the errors were grouped based on recurring patterns and similarities in user behavior. The resulting categories reflect the most frequent types of usability breakdowns encountered during the tasks and were refined iteratively while reviewing observation notes and screen recordings.
...
As can be observed in Table~\ref{tab:nasatlx}, most users have the same patterns, with only a few outliers. P5 experienced a higher mental demand than average. This can be due to the association of mental demand and focus. Nevertheless, this ends up tying with the high performance score. In short, this user was focused on delivering the best. The temporal column reflects what was observed relatively well since nearly all users took less than 10 minutes to finish all tasks in a previously unknown software piece. Effort was loosely tied with mental demands. Furthermore, the scores reflect well the actual tests, for instance, P1 was very relaxed and familiar with \gls{AADL} and \gls{ROS} so he understood and executed the tasks effortlessly as opposed to P3 and P5, which both had the lowest times working with \gls{ROS}. Lastly, frustration was unanimously low, which cemented the good, risk mitigating architecture that not only prevents the user from making certain mistakes but also pushes him in the right direction from the start.

From a usability perspective, the NASA-\gls{TLX} results directly support the evaluation goals defined in Section~\ref{sec:test_op_def_usability}. The consistently low scores in mental demand, temporal demand, and frustration indicate that the configuration workflow imposes limited cognitive overhead on first-time users, even when performing a diverse set of configuration tasks. High perceived performance scores across all participants suggest that users felt confident in their ability to successfully complete the tasks and interpret system feedback correctly.

The low frustration scores across all participants further suggest that the system provided good feedback and error prevention mechanisms, reducing the likelihood of a negative user experience during configuration. Although some participants reported higher mental demand and effort, particularly P5, these scores were accompanied by high perceived performance, suggesting focus instead of cognitive overload. Overall, the workload profile observed aligns with the design goals of minimizing cognitive effort while maintaining control and predictability in the configuration process.
...
Qualitative comments collected during the \gls{SUS} administration suggested that:

\begin{itemize}
	\item Users with prior \gls{AADL} experience tended to rate the system slightly higher.
	\item Scores suggested low perceived workload in the NASA-\gls{TLX} questionnaire, reinforcing the observed low cognitive effort.
\end{itemize}
...
The usability evaluation results collectively suggest that the configuration language and its supporting workflow meet the usability requirements defined in Section~\ref{sec:test_op_def_usability}. Across objective measures (completion time, error frequency, help requests), subjective assessments (\gls{SUS}), and perceived workload (NASA-\gls{TLX}), the system demonstrated good usability characteristics for first time users.

\textbf{Effectiveness} was high, as all participants were able to complete every task and subtask successfully, even when assistance was requested. No critical task failures were observed, and errors did not prevent users from achieving the intended configuration goals. When misunderstandings occurred, they were resolved through clarification rather than task abandonment, indicating that the system supports successful goal completion despite initial user uncertainty. This suggests that the configuration language enables users to accurately express their configuration intent and obtain the expected system behavior.

\textbf{Efficiency} was validated through the overall completion time of all tasks, even in the context of a complete lack of exposure to the system. In terms of tasks that required a significant amount of repetition, such as the triggering of codes, the overall completion times were reduced, indicating that users were able to perform core actions more efficiently after minimal exposure. Longer completion times were primarily associated with tasks requiring textual input or multiple configuration steps, and were attributed to task complexity rather than interaction issues.

\textbf{Learnability} was demonstrated by the rapid progress evident from repeated or conceptually similar tasks, as well as the decrease in hesitation. The fact that short-term learning effects were evident from the session suggests that the users are able to create a correct understanding of the workflow for the configuration with the aid of a mental model, even without any prior training.

\textbf{Error tolerance and recovery} was supported in a positive manner, given that there was a small number of critical errors and that overall, the frustration measures were very low according to the results from the NASA-\gls{TLX}. The issue with errors and help requests was considered to be related to system problems, not user problems, and users generally managed to recover from any confusion or error in a non-disruptive fashion.

The \gls{SUS} results reinforced the overall findings, with an average score in the good/acceptable range and low variability across participants. Combined with the low perceived workload measured by NASA-\gls{TLX}, these results indicate that the system achieves a balance between expressive configuration capabilities and manageable cognitive effort. 

Qualitative observations revealed a small number of concrete design issues, primarily related to naming and option discoverability. Many participants showed uncertainty when locating build related configuration options and generation hooks, often requiring help despite understanding the task goal. This was reflected in both verbal comments and repeated navigational help requests. These issues were not caused by interaction complexity but by ambiguous or overly technical naming, specifically in the use of \textit{Build Integration} and \textit{Extension} hooks. The consistency of these observations across participants indicates a design issue rather than isolated user misunderstanding, which afterwards led to the simplification and regrouping of these options.
...
\begin{table}[htbp]
	...
	\begin{tabular}{p{3cm} p{10cm}}
		...
		P4 & This participant was highly familiar with the Eclipse environment. That knowledge pushed the user to learn more about the software itself on top of just performing the tasks. \\
		P5 & Participant 5 understood the workflow sequence perfectly and praised the intuitiveness of the \gls{DSL} itself. \\
		\bottomrule
	\end{tabular}
\end{table}
...
From the qualitative observations, three recurring themes were identified. 
First, \textbf{environmental interference} was observed, particularly due to the presence of unrelated Eclipse configurations, which caused initial confusion for nearly all participants, although P1 was affected the most. 
Second, \textbf{prior domain and tool familiarity} influenced task fluency, with participants experienced in \gls{AADL} or Eclipse navigating the workflow more confidently (P2, P4). 
Third, a \textbf{mismatch between task phrasing and UI terminology} was noted, where participants expected a direct correspondence between task instructions and configuration option names. This expectation led to increased completion times for tasks such as \textit{Disable launch file generation}, even though the underlying option grouping was logically consistent. 
These themes highlight areas where usability was influenced not by interaction difficulty but by contextual and semantic factors.


\subsection{Threats to Validity}
\label{sec:test_usabl_validity}

Although the tests conducted were very promising and revealed a strong structural integrity of the software built, a core weakness of the validation tests was the lack of a large number of participants capable of using \gls{ROS} in a high degree of proficiency. This was mainly due to the difficulty of recruiting \gls{ROS} industry individuals (our specific target demographic).
...
It is natural that users learned over the completion of the tasks, which in turn led to finishing tasks being completed faster. To mitigate this, the middle section of the tasks (Section B to section E, see Appendix~\ref{app:task_book}) were shuffled in different ways for each participant, which lead to more normalized results. Still, Section F was always evaluated with the experience of all prior tasks, which was mandatory since it was the final code generation step.
...
Sampling was a major issue for the tests, mainly since the tool is targeting a specific industry user, which reduces the participant pool significantly. The sample size ended up being smaller than what many experts consider optimal, however the participants represented the closest to a \gls{ROS} expert user. Still, that representation is not 100\% accurate since none of the users was an actual, multi-year working industry \gls{ROS} expert.

Additionally, participants were recruited through convenience sampling based on availability and proximity to the project, which further limits the generalizability of the results to the broader \gls{ROS} user population.
...
Evaluator bias is also a potential internal validity threat, as the test sessions were conducted by the system developer. Assistance was provided only when participants explicitly requested help. However, the presence of the developer could have influenced participant behavior. This risk was mitigated by treating help requests and errors as indicators of design issues rather than user performance.

A threat to construct validity can be seen from the fact that usability was operationalized primarily through task performance metrics, self-reported usability (\gls{SUS}), and perceived workload (NASA-\gls{TLX}). These measures capture key aspects such as efficiency, perceived ease of use and cognitive effort but they do not cover all facets of usability, such as long term learnability, memorability, or integration into real extended workflows. As a result, the evaluation reflects short term, task oriented usability rather than long term system use.

Finally, conclusion validity is limited by the small sample size, which provides insufficient statistical power to support subgroup comparisons or inferential claims. Observations related to prior experience with \gls{ROS}, \gls{AADL}, or Eclipse are therefore descriptive in nature and should be interpreted as indicative trends rather than statistically supported differences. The study intentionally avoids hypothesis testing and relies on exploratory analysis.


\subsection{Conclusion}
\label{sec:test_usabl_conclusion}

Overall, the usability evaluation indicates that the defined usability goals were met. Participants were able to complete all tasks successfully (effectiveness), did so within acceptable time frames despite having no prior exposure (efficiency) and demonstrated rapid improvement across repeated tasks (learnability). Subjective assessments reinforced even more these findings, with \gls{SUS} scores in the good to excellent range and the overall low perceived workload and frustration according to the NASA-\gls{TLX}.

Many users noted that the \textit{Generation Hooks} were a part of file management, hence should be included in the \textit{Files} submenu. This change, which is shown in Section~\ref{sec:dsl_files}, was not only logical but also simple and practical since it reduced menu clutter and made the overall \gls{DSL} more organize. The same can be said about the renaming of the \textit{Build Integration} section to \textit{Build}. These changes were implemented immediately after testing was finished and aimed to reduced navigational ambiguity, to align option names with user expectations, and decrease menu clutter. Additional insights in task phrasing and discoverability provide guidance for future refinements of the configuration workflow.

Although the results indicate strong usability characteristics, further evaluation with a broader and more diverse population of \gls{ROS} practitioners and within the complete \gls{RAMSES} workflow would provide additional validation of long-term usability and real-world integration. Still, these results already demonstrate that complex code generation customization can be exposed through a configuration \gls{DSL} without imposing excessive cognitive or interaction overhead.
