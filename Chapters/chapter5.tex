...
\begin{table}[htbp]
	...
	\begin{tabular}{lcc}
		...
		\midrule
		Select target language 				& 35.73 & 2 \\
		Select Linux as OS	 				& 6.54 & 2 \\
		Set output directory 				& 21.02 & 1 \\
		Trigger code generation (1)			& 47.78 & - \\
		Configure naming style 				& 29.45 & - \\
		Apply identifier prefix/suffix 		& 32.96 & 2 \\
		Enable comments 					& 70.53 & 2 \\
		Enable report generation			& 20.14 & 1 \\
		Enable logic summaries in report	& 6.68 & - \\
		Enable traceability					& 21.38 & - \\
		Enable traceability JSON			& 14.05 & - \\
		Toggle business logic decoupling 	& 14.70 & - \\
		Change executor type 				& 7.94 & 2 \\
		Configure legacy libraries 			& 21.46 & 1 \\
		Add pre/post scripts 				& 34.23 & 4 \\
		Set file prefix 					& 27.87 & - \\
		Enable file overwriting 			& 6.71 & - \\
		Manage build file generation 		& 43.64 & 3 \\
		Trigger code generation (2)			& 25.11 & - \\
		Verify outputs 						& 42.95 & - \\
		\midrule
		\textbf{Totals} 					&  & 17 \\
		\bottomrule
	\end{tabular}
...
The low frustration scores across all participants further indicate that the system provided good feedback and error prevention mechanisms, reducing the likelihood of a negative user experience during configuration. Although some participants reported higher mental demand and effort, particularly P5, these scores were accompanied by high perceived performance, suggesting focus instead of cognitive overload. Overall, the workload profile observed aligns with the design goals of minimizing cognitive effort while maintaining control and predictability in the configuration process.
...
\begin{itemize}
	...
	\item Scores correlated positively with low perceived workload in the NASA-\gls{TLX} results, reinforcing the observed low cognitive effort.
\end{itemize}
...
The usability evaluation results collectively indicate that the configuration language and its supporting workflow meet the usability requirements defined in Section~\ref{sec:test_op_def_usability}. Across objective measures (completion time, error frequency, help requests), subjective assessments (\gls{SUS}), and perceived workload (NASA-\gls{TLX}), the system demonstrated good usability characteristics for first time users.
...
Although the tests conducted were very promising and revealed a strong structural integrity of the software built, a core weakness of the validation tests was the lack of many participants capable of using \gls{ROS} in a high degree of proficiency. This was mainly due to the difficulty of having contact and time with a \gls{ROS} industry individual (our specific target demographic).
