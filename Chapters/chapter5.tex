...
\begin{tikzpicture}[
		...
		every node/.style={draw, rounded corners, align=center, minimum width=2.5cm, minimum height=8mm}
		]
		
		% Left tree: Configurations
		\node {<Configurations>}
		child {node {<configurations> (global)}}
		child {node {<configurations> (language=C)}};
...
This way, instead of handling multiple files we can, in a single one, manage every single configuration and the same configuration can be used by different generators since each one can find the configuration it needs.
