...
Figure~\ref{fig:eclipse_config_files} shows the simple file management page where the options discussed previously appear.
...
In Figure~\ref{fig:eclipse_config_comments} we can see the different options and how they are presented to the used in the code comments page. These options are disabled/empty by default.
...
Figure~\ref{fig:eclipse_config_trace} shows the different options for traceability, as we saw previously with the case of the report generation (Section~\ref{sec:dsl_report}), the bottom 2 options are conditioned by the first one, meaning that if the first option (Enable traceability generation) is disabled, the other options are deactivated. This behavior can be seen in Figure~\ref{fig:eclipse_config_trace_disabled}
...
This option is one of the core options and can be found in Figure~\ref{fig:eclipse_config_main} as it is a fundamental code generation option.
