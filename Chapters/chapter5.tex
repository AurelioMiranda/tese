...
\begin{xltabular}{\textwidth}{X X X X X}
	...
	P2 & 6 months & Light (Academic) & Brief (Early version) & Accompanied the concept of the \gls{DSL} during development. \\
	P3 & 6 months & Light (Work Research) & None & \gls{RAMSES} developer with no connection to the code generation module. \\
	P4 & 2 years & None & None & TODO. \\
	P5 & none & None & None & TODO. \\
...
The planned an methodical testing process TODO: along with the pilot test: helped mitigate invalid runs entirely, this was very positive since it allowed for a better analysis of results and to the non-existence of discarding rules.
Task time data was gathered using a stop watch, where each task essentially corresponded to a "lap" on the stop watch, then, task times were associated by order. This method, although highly accurate, required full attention to detail since if a task was skipped, the timing would be off on the rest of the tasks, nevertheless, it was very practical since error, misunderstandings and help requests still had to be answered at the same time. Thanks to TODO: the pilot test and: the screen recording of the test, the negative effects of the timing method were greatly reduced and the data was gathered cleanly.
With the data not gathered, the processing consisted of averaging the time taken per task and 
% TODO: Describe how data were cleaned (outliers, invalid runs, discarding rules).
% TODO: Describe aggregation method (mean/median + rationale).
% TODO: Specify device used to measure time (script? stopwatch? logs?).
...
\begin{table}[htbp]
	...
	\begin{tabular}{lccc}
		...
		\midrule
		Select target language &  &  & i \\
		Configure naming style &  &  &  \\
		Apply identifier prefix/suffix &  & i &  \\
		...
		Set file prefix &  &  &  \\
		Manage build file generation &  &  & i \\
		Add pre/post scripts &  &  & i \\
...
% TODO: Add measures of variance (SD/CI).
...
Nevertheless, these errors were quantified in the following Table~\ref{tab:usability_errors}.
...
\begin{table}[htbp]
	...
	\begin{tabular}{lcl}
		...
		\midrule
		Navigation error & 6 & Incorrectly locating the configuration page or submenu. \\
		Misinterpretation & 1 & Misunderstanding the meaning of an option. \\
		Configuration misuse &  & Selecting an option that conflicts with the intended task. \\
		UI confusion & 2 & Confusion caused by labels, layouts, or grouping. \\
		Missing confirmation & 2 & Forgetting to apply/save configuration changes. \\
		Other &  &  \\
		\bottomrule
	\end{tabular}
\end{table}
...
% TODO: Document whether TLX scoring was weighted.
% TODO: Add interpretation of scores (low/medium/high workload thresholds).
...
\begin{table}[htbp]
	...
	\begin{tabular}{lcc}
		\toprule
		\rowcolor{Gainsboro}
		Participant & Adjusted Score (0 - 40) & SUS Score (0 - 100) \\
		\midrule
		P1 & 30 & 75 \\
		P2 & 27 & 67.5 \\
		P3 & 32 & 80 \\
...
\begin{itemize}
	\item \textbf{Mean \gls{SUS} score:} XX
	\item \textbf{Median \gls{SUS} score:} XX
	\item \textbf{Standard deviation:} XX
\end{itemize}
...
Based on these thresholds, the configuration language demonstrated a usability level of \textbf{TODO: in the end ofc}, indicating that participants perceived the workflow as [TODO: here too].
...
\begin{table}[htbp]
	...
	\begin{tabular}{p{3cm} p{10cm}}
		...
		\midrule
		P1 & Participant 1 described that, although the current UI will not translate to the final tool, the fact that other Eclipse configurations are present on top on the current configuration generates confusion. To minimize this problem, subsequent participants were explicitly told that the only configuration they should change was \textit{CodeGen Config}. \\
		P2 &  \\
		P3 &  \\
		P4 &  \\
		P5 &  \\
