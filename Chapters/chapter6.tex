...
The contributions made to the \gls{RAMSES} project, the modified generator, the \gls{DSL} code and their respective documentation, are present in Telecom's GitLab. Although \gls{RAMSES} is an open-source tool, since the second version is still in development, the access to this code is private.


\section{Implications for Model-Driven Engineering Tools}
\label{sec:implications}

The results of this work have implications that extend to other \gls{MDE} tools, concerning the friction between formal rigor and industrial adaptability. While \gls{MDE} tools excel at providing architectural correctness and consistency, they often fail to accommodate the variability and integration constraints imposed by real-world industrial environments.
...
By delegating generation policies into a \gls{DSL} rather than modifying transformation code to accommodate each new requirement, variability is expressed declaratively and interpreted at generation time. This separation of concerns aligns with software engineering principles and suggests that configurability should be considered a more relevant design concern in future \gls{MDE} tools, particularly those targeting safety-critical systems.
...
The design and implementation of the configuration \gls{DSL} provided several practical lessons regarding extensibility in model driven engineering tools. A key insight was that extending existing infrastructures is often more effective than reimplementing core mechanisms, particularly when dealing with mature platforms such as Eclipse. While this study did not include a comparative evaluation against reimplementation approaches, the results suggest that using existing mechanisms allowed users to complete configuration tasks with generally consistent performance and limited need for assistance. These observations indicate that this approach can support usability, although other comparative studies would be required to assess its effectiveness relative to alternative solutions.

The creation of custom components was a core highlight of this implementation. The ability to shape, polish and perfect a already developed solution to a specific problem was a key to success in handling the flow and storage of the \gls{DSL}. Both the custom Acceleo Workflow component and the Custom Preference Store component are examples of situations where its easier to extend an already available feature as opposed to creating one from scratch.

Beyond the more complex and technical challenges, the overall development of the \gls{DSL} provided many learning and improvement opportunities that stemmed from the tools used, the people tied to the project, the constant research, the adoption of alternative methods of implementing features\footnote{\textit{For instance}, the decision to use a custom preference store instead of the default one (See Section~\ref{sec:dsl_custom}).}, and many more.
